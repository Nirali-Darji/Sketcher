...
function PollyLineProperties() {
  ...
  useEffect(() => {
    if (sketcher?.selectedEntity?.mConstructionPoints) {
      const initialPoints = sketcher.selectedEntity.mConstructionPoints.map(
        (point) => ({
          x: point.x,
          y: point.y,
          z: point.z,
  ...
  const handleUpdate = () => {
    if (sketcher?.selectedEntity) {
      ...
      sketcher.selectedEntity.mConstructionPoints = updatedPoints.map(
        (point, index) => ({
          x: point.x,
          y: point.y,
          z: point.z,
        ...
        {points.slice(0, points.length - 2).map((point, index) => (
          <div key={index}>
            ...
            <InputBox
              label={"x"}
              value={point.x}
              onChangeHandler={handlePointChange(index, "x")}
            />
            <InputBox
              label={"y"}
              value={point.y}
              onChangeHandler={handlePointChange(index, "y")}
            />
            <InputBox
              label={"z"}
              value={point.z}
              onChangeHandler={handlePointChange(index, "z")}
            />
          </div>
        ))}
      </div>
